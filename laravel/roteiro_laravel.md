...
### Configurando o IDE helper generator para ajudar no projeto:

Na raiz do projeto, rode o comando

    composer require barryvdh/laravel-ide-helper

registre o serviço em `config/app` array providers

    Barryvdh\LaravelIdeHelper\IdeHelperServiceProvider::class,

rode o comando artisan

    php artisan ide-helper:generate

pronto, seu ide agora vai ter autocomplete dos metodos laravel

### Adicionando barra de debbug ao projeto (app_debug === true)

    composer require barryvdh/laravel-debugbar –dev wasd
...
Não mexer no `id` e nas `timestamps` que já estão la. Add duas colunas no meio
...
todos os controllers que não forem do acesso público do site devem ter o midleware auth no construct.
...
Você cadastra no controller assim:

```php
\Session::flash('mensagem', 'Post editado com sucesso.');

// Na view você recebe assim

@if(Session::has('mensagem'))
    <div class="alert alert-warning text-center">
        <p><b>{{Session::get('mensagem')}}</b></p>
    </div>
 @endif
```
